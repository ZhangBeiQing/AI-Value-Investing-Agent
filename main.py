...
async def main(config_path=None, force_run=False):
    """Run trading experiment using BaseAgent class
    
    Args:
        config_path: Configuration file path, if None use default config
        force_run: If True, force run for the specified date range regardless of existing records
    """
    ...
    initial_cash = agent_config.get("initial_cash", 500000.0)
    ...
    for model_config in enabled_models:
        ...
        model_extra_body = model_config.get("extra_body")
        ...
        try:
            ...
            await agent.run_date_range(INIT_DATE, END_DATE, force_run=force_run)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='AI Trader Main Entry')
    # positional argument for config, optional
    parser.add_argument('config_path', nargs='?', default=None, help='Path to configuration file')
    # flag for force-run
    parser.add_argument('--force-run', action='store_true', help='Force run for the specified date range')
    ...
    asyncio.run(main(config_path, force_run=force_run))
