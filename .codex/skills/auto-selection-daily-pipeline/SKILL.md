...
即**最近一个已收盘的交易日**，默认 `today`。

- 典型节奏：当天晚上 9 点（A 股 15:00 收盘后）跑当天收盘数据，`YYYY-MM-DD = 今天`
- 用户说"今天自动选股"时，`YYYY-MM-DD = today`。**不要减一**，「默认昨天」是已废弃的旧口径
- 若"今天"是周末或节假日，向前查找最近一个交易日，并把结论告知用户
- "上一交易日"指相对于 `run_date` 的前一个交易日，不是相对于日历今天
  - 例：`run_date=2026-04-22` → 上一交易日 = `2026-04-21`（而非 `2026-04-21` 之前再往前退）
- 开始执行前，如果用户没有显式指定日期，先用 `date +%F` 或等价方式取当天，并向用户确认一次再继续
...
1. 解析日期：按 Section 2 的日期语义确定 `YYYY-MM-DD = 要分析的交易日`（默认 `today`，用户没显式指定时先推导并向用户确认）
