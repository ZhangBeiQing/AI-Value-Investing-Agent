...
#### 8.3 主agent评审回看所有股票的深度分析结果
主 agent 全部的完整的阅读每个 P0 subagent 的 `*_decision.json`，评审其分析结论的合理性和一致性，不要担心上下文不够，我们有1M的上下文，完全够用。
尤其关注庭审正反双方的观点是否充分，是否遗漏了关键证据，法官的判决是否合理。然后在规则合规、可执行性、价格印象自洽、置信度等维度进行分别评审


#### 8.4 向用户展示汇总结果
主agent评审完后向用户展示汇总最终股票的深研结果
明确分类呈现：
