...
长期池采用“严格准入、分批建仓、有效初仓、证伪退出”，避免把长期投资误写成长期等待或用象征仓位回避决策：

1. **标的准入要严**：只有基本面质量、估值或价格印象、长期核心逻辑与主要风险均通过庭审，才允许进入 `BUY`。入选长期池或命中热点主题不等于自动买入。
2. **买点不必完美**：长期逻辑和安全边际已经成立时，短期技术面不完美通常应通过分批建仓处理，不宜要求所有量价、宏观和催化信号同时确认后才开始买入。
3. **初始仓位必须有意义**：仓位按输入中的真实总资产计算。除非预测可靠度低或处于高风险事件窗口，否则不宜用不足总资产 1% 的象征仓位冒充完成建仓；应明确当前仓位、首批仓位、计划目标仓位及各自占总资产比例。
4. **确认后加仓，证伪则退出**：首批仓位用于建立真实敞口；后续只有在基本面兑现、估值仍合理或量价确认后加仓。若长期逻辑、盈利能力或风险边界被证伪，应减仓或退出，不得把“长期持有”当作拒绝纠错。
...
1. data/skill_runs/{date}/long_book/03_agent_input.md
2. data/skill_runs/{date}/long_book/01_global_context.md
3. data/selection_runs/{date}/06_hot_news_state.json
4. data/selection_runs/{date}/05_board_heat_digest.json
5. data/skill_runs/{date}/long_book/04_stock_research/{stock_name}_{symbol}_{date}_research.md
...
## 输出
将结果写入文件：data/skill_runs/{date}/long_book/subagent_result/{stock_name}_{symbol}_{date}_decision.json
