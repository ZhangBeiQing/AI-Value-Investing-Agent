...
3. **先判断，再估值**：先回答四个问题，再决定是否需要估值：
   - 今日是否允许重算估值锚
   - 盈利预测可靠度是 `high` / `medium` / `low`
   - 估值模式是 `simple_valuation` / `range_valuation` / `no_precise_valuation`
   - 最终动作候选是什么
4. **预测可靠度低时从严**：若 `low`，默认不给新的 `BUY` 或加仓；此时重点是判断是否继续持有、减仓或退出。
5. **SOTP 只在必要时使用**：只有业务拆分会实质影响结论、且关键分部假设可验证时，才允许使用 SOTP；能整体估值就不要硬拆。
6. **禁止事项**：
   - 不得读取其他股票研究包
