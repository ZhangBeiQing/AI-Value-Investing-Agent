...
from agent_tools.tool_financial_report import (
    get_financial_report_summary as _tool_get_financial_report_summary,
)
...
from agent_tools.tool_stock_news_search import search_stock_news as _tool_search_stock_news
...
search_stock_news = getattr(_tool_search_stock_news, "fn", _tool_search_stock_news)
get_financial_report_summary = getattr(
    _tool_get_financial_report_summary,
    "fn",
    _tool_get_financial_report_summary,
)
...
def _research_output_path(symbol: str, run_date: str, output_dir: Path) -> Path:
    symbol_info = parse_symbol(symbol)
    ...
    target_dir = output_dir / "04_stock_research"
...
def main() -> None:
    ...
    for entry in TRACKED_A_STOCKS:
        content = build_research_markdown(entry.symbol, args.run_date)
        _research_output_path(entry.symbol, args.run_date, output_dir).write_text(
            content,
            encoding="utf-8",
        )
