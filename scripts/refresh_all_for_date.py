#!/usr/bin/env python3
"""一键刷新某个交易日的全部 Python 链路数据，并打印后续 skill 清单。
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="一键刷新某个交易日的宏观/行情/新闻/板块/公告/选股输入等 Python 链路数据。",
    )
    parser.add_argument(
        "--date",
...
def main() -> int:
    ...
    LOGGER.info(
        "开始一键刷新：date=%s, fresh_heavy=%s, max_workers=%d",
        run_date,
        args.fresh_heavy,
        args.max_workers,
    )
    ...
    if result.succeeded:
        print(format_followup_checklist(run_date))
