...
归根结底四个字：**低买高卖**。但"低"和"高"怎么定义，取决于公司类型。我们把公司分成三类、对应两套估值框架：

**框架 A：历史均值估值**（周期型 + 稳定型）。估值锚是过去——历史 PE/PB/价格区间是已知的，"便宜"和"贵"有明确参照。分批建仓的策略是越跌越买、摊薄成本。

**框架 B：产业链远景估值**（爆发型）。估值锚是未来——天花板和增长路径是推算出来的。"便宜"不是看当前 PE 多低，而是看离天花板还有多远、增长还能消化多少估值。这套方法来自基金经理**程浩然**：

1. **选行业**：找到需求爆发、总量还有数倍增长空间的产业链
2. **选龙头**：在供需错配的环节里挑龙头——信息透明、资金容量大、涨得不比行业少、跌得比行业少
3. **买在基本面右侧**：不猜市场底，基本面改善、赔率胜率够就买入
4. **长线持有**：产业周期 3-5 年，中途跌 20%、50% 不代表错，产业趋势不变就不丢仓位
5. **到天花板卖出**：景气度恶化或触及理论市值天花板时果断离场

在框架 B 下，分批建仓的策略不是"越跌越买"，而是随产业趋势验证逐步加大仓位。

两套框架看似矛盾——一个强调价格、一个淡化价格——实际上是因为估值锚不同。归根结底都是低买高卖，区别只在"低"是相对历史均值还是相对产业天花板。

产业链研究的结论来自财报深研阶段，结果写入研究包供决策时参考。完整策略见 `configs/prompt_flow/fixed_tracked/investment_policy.md`。
